...
public class ElevatorImpl implements Runnable, Elevator {
    ...
    /*
     * The method by which the Elevator accepts calls from the controller.
     */
...
/*
    ...
    public void pushButton(int floor) {
        ...
        else desiredDirection = Direction.UP;
        ...
        synchronized (requests){
            ...
            if ((desiredDirection == direction) ||
                ...
                addRequest(request);
                System.out.printf("%s Elevator %d accepting a(n) %s button press to floor %d %s\n",
                        Main.currentTime(), elevatorID, desiredDirection, request.getTargetFloor(), requestString(requests));
            } else {
                System.out.printf("%s Elevator %d ignoring a(n) %s button press to floor %d %s\n",
                        Main.currentTime(), elevatorID, desiredDirection, request.getTargetFloor(), requestString(requests));
            }
    ...
    private void returnHome(){
        Request goHome = new Request(this.getCurrentFloor(), 1);
        synchronized (requests) {
            currentRequest = null;
            addRequest(goHome);
        }
    ...
    public void call(Request request) {
        System.out.printf ("%s Elevator %d accepted call to floor %d %s\n",Main.currentTime(), elevatorID, request.getTargetFloor(), requestString(requests));
        addRequest(request);
    }
